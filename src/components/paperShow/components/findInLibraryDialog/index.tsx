...
import { closeFindInLibraryDialog } from '@src/reducers/findInLibraryDialog';
...
import PaperAPI from '@src/api/paper';
  ...
  return (
    <Dialog open={isOpen} onClose={handleClose} classes={{ paper: s.dialogContainer }}>
      ...
      <div className={s.bottomBanner}>
        Request Paper is not available.
        <br />
        Please use the new request form <span className={s.highLightKeyword}>to ask your library.</span>
      </div>
    </Dialog>
  );
};
