...
import SearchQueryManager, {
  FilterObject
} from "../../helpers/searchQueryManager";
...
const SearchResult: React.FC<Props & {
  ...
  const hasNoSearchResultAndNoAuthorResult =
    hasNoSearchResult && hasNoMatchedAuthors;
      ...
      <NoSsr>
        <div className={styles.innerContainer}>
          <DoiSearchBlocked
            ...
            searchDoi={articleSearchState.doi || ""}
          />
        </div>
      </NoSsr>
    );
    ...
    return (
      <div className={styles.innerContainer}>
        <div
          ...
          style={shouldAddMargin ? { marginTop: "32px" } : {}}
        >
        ...
        <SearchList
          ...
          isLoading={articleSearchState.isContentLoading}
          searchQueryText={
            articleSearchState.suggestionKeyword || queryParams.query || ""
          }
        />
        <Pagination
...
const SearchContainer: React.FC<Props> = props => {
  ...
  const [filter, setFilter] = React.useState(
    SearchQueryManager.objectifyPaperFilter(queryParams.filter)
  );
  ...
  React.useEffect(() => {
    ...
    changeSearchQuery({
      query: SafeURIStringHandler.decode(currentQueryParams.query || "")
    });
    setQueryParams(currentQueryParams);
    setFilter(
      SearchQueryManager.objectifyPaperFilter(currentQueryParams.filter)
    );

    // set params
    const params = SearchQueryManager.makeSearchQueryFromParamsObject(
      currentQueryParams
    );
    params.cancelToken = cancelToken.current.token;
    params.detectYear =
      articleSearchState.searchInput !== currentQueryParams.query ||
      enableAutoYearFilter;
  ...
  const shouldShowEmailBanner =
    !currentUserState.isLoggedIn && !articleSearchState.isContentLoading;
        ...
        <div className={styles.leftBoxWrapper}>
          <AuthorSearchResult
            ...
            shouldShow={
              ...
              SearchQueryManager.isFilterEmpty(filter)
            }
          />
