...
import { immutablyRemoveItemFromArray } from '../helpers/immutablyRemoveItemFromArray';
...
export interface SearchFilterState extends AggregationData {
  ...
  sorting: PAPER_LIST_SORT_OPTIONS;
...
export function reducer(state = SEARCH_FILTER_INITIAL_STATE, action: SearchActions) {
  switch (action.type) {
    ...
    case ACTION_TYPES.ARTICLE_SEARCH_SELECT_JOURNAL_FILTER_ITEM: {
      return {
        ...state,
        selectedJournalIds: immutablyRemoveItemFromArray(action.payload.journalId, state.selectedJournalIds),
      };
    }
    ...
    case ACTION_TYPES.ARTICLE_SEARCH_SELECT_FOS_FILTER_ITEM: {
      return {
        ...state,
        selectedFOSIds: immutablyRemoveItemFromArray(action.payload.FOSId, state.selectedFOSIds),
      };
    }
