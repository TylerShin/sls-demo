...
import { immutablyRemoveItemFromArray } from '@src/helpers/immutablyRemoveItemFromArray';
...
export interface CollectionShowState
  extends Readonly<{
    ...
    sortType: AUTHOR_PAPER_LIST_SORT_OPTIONS;
...
export function reducer(
  ...
  switch (action.type) {
    ...
    case ACTION_TYPES.COLLECTION_SHOW_SUCCEEDED_GET_PAPERS: {
      ...
      return pageRes
        ? {
            ...
            isLoadingPaperToCollection: false,
            paperIds,
            sortType: action.payload.sort as AUTHOR_PAPER_LIST_SORT_OPTIONS,
    ...
    case ACTION_TYPES.GLOBAL_START_TO_REMOVE_PAPER_FROM_COLLECTION: {
      if (action.payload.collection.id === state.mainCollectionId) {
        ...
        if (typeof state.paperIds === 'object') {
          const newPaperIds = state.paperIds.filter(id => !removePaperIds.includes(id));
          const newSelectedPaperIds = state.selectedPaperIds.filter(id => !removePaperIds.includes(id));
    ...
    case ACTION_TYPES.COLLECTION_SHOW_SELECT_PAPER_ITEM: {
      return {
        ...state,
        selectedPaperIds: immutablyRemoveItemFromArray(action.payload.paperId, state.selectedPaperIds),
      };
    }
