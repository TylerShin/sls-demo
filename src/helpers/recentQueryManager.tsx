import { getWordsArraySplitBySpaceWithoutStopWords } from "../helpers/getWordsWithoutStopWords";
...
export function getRecentQueries(searchText: string): SuggestionItem[] {
  ...
  const matchedQueries = recentQueries.filter(query => {
    return query
      ...
      .map(q => q.trim())
      .some(q =>
        textToMatch.some(text => q.toLowerCase().startsWith(text.toLowerCase()))
      );
  });
...
export function saveQueryToRecentHistory(searchText: string) {
  ...
  if (i > -1) {
    newQueries = [
      searchText,
      ...oldQueries.slice(0, i),
      ...oldQueries.slice(i + 1)
    ];
  } else {
